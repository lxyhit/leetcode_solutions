...
public:
    ...
    int get(int index) {
        if(index < _length && index >= 0){
            LinkedNode* result = _dummyHead;
            for(int tag=0; tag<=index; tag++){
                result = result->next;
            }
            return result->val;
        }else{
    ...
    void addAtTail(int val) {
        LinkedNode *tag = _dummyHead;
        while(tag->next != nullptr){
            tag = tag->next;
        }
    ...
    void deleteAtIndex(int index) {
        if(index >= 0 && index < _length){
            ...
            LinkedNode *toBeDeleted = tag->next;
